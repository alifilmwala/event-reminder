...
function mapGuest(r: {
  ...
  token: string; linkVisited: boolean; visitedAt: Date | null;
  createdAt: Date;
}): Guest {
  return {
    ...r,
...
function mapMessage(r: {
  id: string; guestId: string; status: string;
  sentAt: Date | null; errorMsg: string | null;
  retries: number; createdAt: Date;
}): Message {
  return {
    ...r,
...
function mapSchedule(r: {
  id: string; scheduledAt: Date; type: string;
...
export async function addGuest(input: {
  ...
  if (existing) {
    const updated = await prisma.guest.update({
      ...
      data:  { name: input.name, tableNumber: input.tableNumber },
    });
  ...
  const guest = await prisma.guest.create({
    data: {
      ...
      messages:    { create: { status: 'PENDING' } },
    },
  });
...
/**
 * Bulk upsert — adds new guests and updates existing mobile numbers.
 */
export async function addGuestsBulk(
  inputs: Array<{ name: string; mobile: string; tableNumber: string }>,
): Promise<{ added: number; updated: number; guests: Guest[] }> {
  let added   = 0;
  let updated = 0;
  ...
  for (const input of inputs) {
    ...
    if (existing) {
      ...
      updated++;
      resultGuests.push(mapGuest(row));
    } else {
...
export async function updateGuest(
  id: string,
  data: Partial<Pick<Guest, 'name' | 'tableNumber' | 'linkVisited' | 'visitedAt'>>,
): Promise<Guest | null> {
  try {
    const row = await prisma.guest.update({
      where: { id },
      data: {
        ...
        visitedAt: data.visitedAt !== undefined
          ? (data.visitedAt ? new Date(data.visitedAt) : null)
          : undefined,
      },
    });
    return mapGuest(row);
...
export async function updateMessage(
  id: string,
  data: Partial<Omit<Message, 'id' | 'createdAt' | 'guestId'>>,
...
export async function getEvent(): Promise<EventConfig> {
  const row = await prisma.eventConfig.upsert({
    where:  { id: 'default' },
    update: {},
    create: { id: 'default', ...DEFAULT_EVENT_DATA, date: new Date(DEFAULT_EVENT_DATA.date) },
  });
...
export async function updateEvent(data: Partial<Omit<EventConfig, 'id'>>): Promise<EventConfig> {
  const row = await prisma.eventConfig.upsert({
    ...
    create: { id: 'default', ...DEFAULT_EVENT_DATA, ...data, date: data.date ? new Date(data.date) : new Date(DEFAULT_EVENT_DATA.date) },
  });
  return { ...row, date: row.date.toISOString() };
}
...
export async function updateSchedule(
  id: string,
  data: Partial<Pick<Schedule, 'status'>>,
...
export async function getStats(): Promise<DashboardStats & { recentActivity: { day: string; sent: number }[] }> {
  const [totalGuests, linkVisitedCount] = await Promise.all([
    prisma.guest.count(),
    prisma.guest.count({ where: { linkVisited: true } }),
  ]);

  // Guests that have at least one SENT message
  const sentGuestIds = await prisma.message.findMany({
    where:  { status: 'SENT' },
    select: { guestId: true },
    distinct: ['guestId'],
  });
  const sentSet   = new Set(sentGuestIds.map((r) => r.guestId));
  const sentCount = sentSet.size;

  // Pending = guests with no SENT message
  const pendingCount = totalGuests - sentCount;

  // Failed = latest message per guest is FAILED and guest has no SENT message
  const latestMsgs = await prisma.$queryRaw<{ guestId: string; status: string }[]>`
    SELECT DISTINCT ON ("guestId") "guestId", status
    FROM messages
    ORDER BY "guestId", "createdAt" DESC
  `;
  const failedCount = latestMsgs.filter(
    (m) => m.status === 'FAILED' && !sentSet.has(m.guestId),
  ).length;
  ...
  for (const msg of recentMsgs) {
    if (msg.sentAt) {
      const day = msg.sentAt.toISOString().slice(0, 10);
